...
window.handleRegister = async function(event) {
  ...
  if (!/^[a-zA-Z0-9_]{3,32}$/.test(username)) {
    errEl.textContent = 'ชื่อผู้ใช้ต้องเป็น a-z, 0-9 หรือ _ จำนวน 3-32 ตัว';
    return;
  }
  ...
  try {
    ...
    errEl.textContent = err.response?.data?.error || 'ไม่สามารถสมัครสมาชิกได้';
  } finally {
    btn.disabled = false;
...
window.rentBot = async function(days, diamonds) {
  ...
  try {
    ...
    currentUser.expiresAt = res.data.expiresAt || res.data.botExpiry;
...
window.handleSlipPreview = function(event) {
  ...
  if (!allowedTypes.includes(file.type) || file.size > 5 * 1024 * 1024) {
    event.target.value = '';
    showToast('ไฟล์ต้องเป็น JPG, PNG หรือ WEBP และมีขนาดไม่เกิน 5MB', 'error');
    return;
  }
...
window.handleSlipSubmit = async function(event) {
  ...
  if (!Number.isInteger(amount) || amount < 1 || amount > 100000) {
    msg.textContent = '❌ กรุณากรอกจำนวนเงิน 1-100,000 บาท';
    return;
  }
  ...
  formData.append('amountBaht', String(amount));
